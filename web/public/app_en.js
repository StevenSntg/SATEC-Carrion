/* SATEC — Carrión's disease outbreak risk map by province of Peru (English UI). */
(function () {
  "use strict";
  ...
  var LABEL = { alto: "high", medio: "medium", bajo: "low" };
  var nivelColor = function (info) { return info && NIVEL[info.nivel] ? NIVEL[info.nivel] : NA; };
  var nivelLabel = function (n) { return LABEL[n] || n; };
  ...
  function showDetail(f, riesgo) {
    ...
    el("detail-depto").textContent = f.properties.FIRST_NOMB || "Peru";
    el("detail-prov").textContent = f.properties.NOMBPROV || "Province";
    var badge = el("detail-nivel");

    if (info) {
      var p = Math.max(0, Math.min(1, info.prob_rn));
      badge.textContent = nivelLabel(info.nivel);
      ...
      var ad = el("detail-ad");
      ad.textContent = info.pred_ad ? "outbreak" : "no outbreak";
      ad.className = "pred-chip" + (info.pred_ad ? " is-brote" : "");
      el("detail-casos").textContent = info.casos;
    } else {
      badge.textContent = "no data"; badge.className = "nivel-badge";
  ...
  Promise.all([
    ...
    var layer = L.geoJSON(geo, {
      ...
      onEachFeature: function (f, lyr) {
        var info = riesgo[ubigeo(f)];
        var nivel = info ? nivelLabel(info.nivel) : "no data";
        var prob = info ? Math.round(info.prob_rn * 100) + "%" : "n/a";
        lyr.bindTooltip(
          "<b>" + (f.properties.NOMBPROV || "") + "</b><br>" +
          "<span class='tt-nivel' style='color:" + nivelColor(info) + "'>Risk " + nivel + "</span> &middot; NN " + prob,
    ...
    if (hint) hint.textContent = "Could not load map data (" + err + ").";
  });
})();
